...
- `src/sudoku.rs` — `Board<const N: usize>` and `Row<N>` (fixed-size arrays, `Copy`). Board I/O (`read`), `overlay` (candidate solution over the puzzle's zero cells), and `fitness` (u16 sum of duplicate counts across rows, columns (strided in-place scan), and boxes; 0 = solved). `src/sudoku/inner.rs` has the bitmask-based `Scorer` used for duplicate counting.
